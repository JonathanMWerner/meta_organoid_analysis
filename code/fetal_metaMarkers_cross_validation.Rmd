...
test_fetal_meta_markers = make_meta_markers(test_markers, detailed_stats = TRUE)
...
plot_pareto_markers(test_fetal_meta_markers, 'Microglia/Immune', min_recurrence = 0)
```
...
for(i in 1:length(human_fetal_dataset_paths)){
  ...
  agg_de_stats = agg_de_stats %>% filter(cell_type != 'other')
  
  
  test_cell_type = c('Glutamatergic','GABAergic','Neural_Progenitor','Non-neuronal','Dividing_Progenitor', 'Intermediate_Progenitor', 'Microglia/Immune')
  ...
  for(cell in test_cell_type){
    ...
    for(j in 1:length(numMarkers)){
      #dups =  agg_de_stats %>% filter(rank <= numMarkers[j]) %>% pull(gene)
...
save(metaMarker_auroc_df, 
     file = 'fetal_metaMarker_cross_validation/6_26_24/cross_dataset_auroc_class_results_metaMarker_all_fetal.Rdata')
```


```{r}
load(file = 'fetal_metaMarker_cross_validation/6_26_24/cross_dataset_auroc_class_results_metaMarker_all_fetal.Rdata')
```
...
path = 'graphs/fetal_metaMarker_cross_validation/6_26_24/roc_pr_curves/', 
device = 'pdf', useDingbats = F, height = 6, width = 10)

ggplot(filter(metaMarker_auroc_df, celltype == 'Microglia/Immune'), aes(x = num_markers, y = auroc, fill = num_markers)) + geom_boxplot()  +
  ylim(.5, 1) + scale_fill_manual(values = magma(6)) + geom_hline( yintercept = .5, color = 'red', linetype = 'dashed') +
  xlab('Number of Fetal MetaMarkers') + ylab('Predicting Fetal cell-types (AUROC)') + ggtitle('Microglia/Immune')
...
metaMarker_auroc_df %>% group_by(celltype, num_markers) %>% summarize(median = median(auroc, na.rm = T), perc25 = quantile(auroc, probs = c(.25), na.rm = T), 
                                                                      perc75 = quantile(auroc, probs = c(.75), na.rm = T)) %>% filter(num_markers == 10)
```
...
metaMarker_auroc_df %>% group_by(celltype, num_markers) %>% summarize(median = median(auroc, na.rm = T), perc25 = quantile(auroc, probs = c(.25), na.rm = T), 
                                                                      perc75 = quantile(auroc, probs = c(.75), na.rm = T)) %>% filter(num_markers == 100)
```
...
micro_fetal_1v1_mat = matrix(nrow = length(human_fetal_dataset_paths), ncol = length(human_fetal_dataset_paths), dimnames= list(fetal_ages, fetal_ages))
...
for(i in 1:length(human_fetal_dataset_paths)){
  ...
  for(j in 1:length(test_markers)){
    ...
    micro_fetal_1v1_mat[i, j] = auc_agg_predictor(dataset, current_de_markers, 'Microglia/Immune', metric = rank_metric, num_markers = n_markers)
...
save(nProg_fetal_1v1_mat, divProg_fetal_1v1_mat, intProg_fetal_1v1_mat, gaba_fetal_1v1_mat, glut_fetal_1v1_mat, nonN_fetal_1v1_mat,
     file = '../data/fetal_1v1/6_26_24/cell_type_1v1_mats.Rdata')
```

Add the leave one out MetaMarkers results to the matrices

```{r}
load(file = '../data/fetal_1v1/6_26_24/cell_type_1v1_mats.Rdata')
```
...
path = 'graphs/fetal_metaMarker_cross_validation/6_26_24/fetal_1v1/', 
...
```{r}
save(agg_exp_df , 
     file = 'fetal_metaMarker_cross_validation/6_26_24/cross_dataset_aggregated_exp_metaMarker_all_fetal.Rdata')
```


```{r}
load(file = 'fetal_metaMarker_cross_validation/6_26_24/cross_dataset_aggregated_exp_metaMarker_all_fetal.Rdata')
```
...
ggsave(filename = 'top_100_gaba_metaMarkers_avg_exp_all_fetal.pdf', 
       path = 'graphs/fetal_metaMarker_cross_validation/6_26_24/agg_violin_metamarker/', 
...
grouped_annotations[grouped_annotations %in% c('Astro','Astrocyte', 'Glioblast')] = 'Astrocyte'
...
```{r}
all_structures = c("GE", "hypothalamus","neocortex","striatum","thalamus","allocortex","claustrum", "cerebellum","midbrain","proneocortex")
```
...
for(i in 1:length(secondTri_human_fetal_dataset_names )){
  ...
  file_path = sprintf('graphs/fetal_metaMarker_cross_validation/6_26_24/regional_marker_exp/%s_regional_exp_heatmap_v2.pdf', current_dataset)
...
save(matrix_list, file = '../data/regional_marker_exp/6_26_24/areal_dataset_regional_marker_exp_matrices.Rdata')
...
save(matrix_list, file = '../data/regional_marker_exp/6_26_24/linnarsson_dataset_regional_marker_exp_matrices.Rdata')
